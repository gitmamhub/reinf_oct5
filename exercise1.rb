...
my_hash[:response][:results].each do |number|
number[:views] = 0
end



 def read_article(my_hash)
   no_article = my_hash[:response][:results].length
   random_article = rand(0...no_article+1)
   my_hash[:response][:results][random_article-1][:views] +=1
   return my_hash
 end
...
def display_views (my_hash)

  my_hash[:response][:results].each do |article|
    puts "article section_name: #{article[:sectionName]}"
...
50.times do
read_article(my_hash)
end

display_views(my_hash)
